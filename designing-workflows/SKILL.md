...
Phase 0/3 的 gate 和 Phase 5 的访谈都假设有用户在场确认。当要求**一次性产出**（没有交互对象）时，不准卡住、也不准悄悄跳过 gate，按下面走：

- 用**显式默认值**推进，绝不默默假设
- 每个默认值在 `gaps.md` 留痕，并标出**最不确定**的那条，提示首次运行核对
- 在交付开头声明"以默认值越过 gate，均可覆盖"
- 这仍是"以终为始"的精神：默认值就是把终点先显式钉下来，等用户确认或覆盖

**违反：** 默默假设而不留痕 = 跳过 gate，违反本 skill。
