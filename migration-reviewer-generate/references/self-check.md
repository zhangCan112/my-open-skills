...
- [ ] **Adapter relocation only**: the scene was partitioned into `adapter core` (full methodology vs the legacy core; acquisition seams tagged `RE-POINTED` and each verified; byte-identity claimed only in the zero-coupling special case, via a byte-oracle) vs `host glue` (oracle = host B's contract, NOT legacy A's glue; divergence from A while conforming to B is reported `INTENDED`, never `DIFFERS`); host B's contract was confirmed to exist — or flagged as a prerequisite deliverable when absent.
...
- [ ] The checklist is **migration-type specific** — it has rows that only make sense for this kind of migration (e.g. hooks/effects for React, integer width for cross-language, data custody for service split, dual-oracle rows — `RE-POINTED` seams, `INTENDED` glue divergence, B-touchpoint MISSING, legacy-A residue — for adapter relocation). The six generic categories alone are a FAIL.
- [ ] **Adapter relocation only**: report rows carry an `oracle` column (`A-core` / `B-contract`); core behaviors B cannot host map preserve/translate/degrade/drop; no byte-equality claim without a byte-oracle harness.
